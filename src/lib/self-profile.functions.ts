...
const selfSchema = z.object({
  ...
  national_id_expiry: z.string().trim().min(1).nullable().optional(),
  passport_no: z.string().trim().max(40).nullable().optional(),
  passport_expiry: z.string().trim().min(1).nullable().optional(),
...
export const updateMyProfile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await supabaseAdmin.from("employees").update(patch).eq("id", me.id);
    if (error) throw new Error(error.message);
